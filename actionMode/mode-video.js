...
(async function switchToVideo() {
  ...
  try {
    ...
    // 2. Tìm và click tab "Video"
    const {result: tabBtnRes} = await Runtime.evaluate({
      expression: `
        (() => {
          const tabs = Array.from(document.querySelectorAll('button[role="tab"]'));
          const btn = tabs.find(b => b.textContent.toLowerCase().includes('video'));
          if (!btn) return null;
          const r = btn.getBoundingClientRect();
          return JSON.stringify({x: r.left + r.width/2, y: r.top + r.height/2});
        })()
    ...
    if (!tabBtnRes.value) throw new Error('Không tìm thấy tab "Video" trong menu!');
    ...
    await sleep(800);
    console.log('  → ✓ Đã chuyển sang mode VIDEO thành công!\n');
