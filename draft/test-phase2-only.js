...
// ── FFmpeg blur config — chỉ che vùng watermark "Veo" góc dưới phải ──────────
// Điều chỉnh 4 giá trị % bên dưới nếu vùng blur bị lệch:
//   BLUR_X_PCT  : % từ trái đến điểm bắt đầu vùng blur (tăng = dịch phải)
//   BLUR_Y_PCT  : % từ trên xuống điểm bắt đầu vùng blur (tăng = dịch xuống)
//   BLUR_W_PCT  : % chiều rộng vùng blur (tăng = rộng hơn)
//   BLUR_H_PCT  : % chiều cao vùng blur (tăng = cao hơn)
const BLUR_RADIUS = 25;  // độ mạnh blur
const BLUR_X_PCT  = 82;  // % từ trái
const BLUR_Y_PCT  = 92;  // % từ trên
const BLUR_W_PCT  = 18;  // % chiều rộng
const BLUR_H_PCT  =  8;  // % chiều cao
...
// Tên ảnh giả (dùng cho tên thư mục output) — lấy từ tên thư mục output
...
async function waitForVideos(Network, Runtime, existingVideoUUIDs = new Set()) {
  ...
  return new Promise((resolve) => {
    ...
    // DOM polling + stall detection (nếu count không tăng trong 30s thì resolve sớm)
    ...
    const pollId = setInterval(async () => {
      ...
      try {
        ...
        // Stall: có ít nhất 1 video nhưng count không tăng trong STALL_MS
...
// ═══════════════════════════════════════════════════════════════
// FFMPEG: BLUR VÙNG WATERMARK "Veo" GÓC DƯỚI PHẢI
// Chỉ blur đúng vùng nhỏ chứa chữ Veo, không đụng phần còn lại
// ═══════════════════════════════════════════════════════════════
function blurVideoBottom(filePath) {
  ...
  if (probe.status !== 0) {
    log(`  ⚠️ ffprobe lỗi: ${probe.stderr?.toString().trim() || probe.error?.message}`);
    log(`  ⚠️ Bỏ qua blur: ${path.basename(filePath)}`);
    return false;
  }
  const [w, h] = probe.stdout.toString().trim().split(',').map(Number);
  if (!w || !h) { log(`  ⚠️ Không đọc được kích thước video`); return false; }

  // Tính toán vùng blur theo %
  const bx = Math.round(w * BLUR_X_PCT / 100);
  const by = Math.round(h * BLUR_Y_PCT / 100);
  const bw = Math.round(w * BLUR_W_PCT / 100);
  const bh = Math.round(h * BLUR_H_PCT / 100);

  // Đảm bảo không vượt quá kích thước video
  const safeW = Math.min(bw, w - bx);
  const safeH = Math.min(bh, h - by);

  if (safeW <= 0 || safeH <= 0) {
    log(`  ⚠️ Vùng blur nằm ngoài video (${bx},${by} ${safeW}×${safeH}), bỏ qua`);
    return false;
  }
  ...
  const filter  = [
    ...
    `[blur_src]crop=${safeW}:${safeH}:${bx}:${by},boxblur=luma_radius=${BLUR_RADIUS}:luma_power=5[blurred]`,
    `[base][blurred]overlay=${bx}:${by}[out]`
  ].join(';');
  ...
  if (result.status !== 0 || !fs.existsSync(tmpPath)) {
    log(`  ⚠️ ffmpeg blur lỗi: ${result.stderr.toString().split('\n').slice(-3).join(' ')}`);
    try { fs.unlinkSync(tmpPath); } catch {}
    return false;
  }

  fs.renameSync(tmpPath, filePath);
  log(`  ✓ Blur "Veo": ${path.basename(filePath)} (${w}×${h} → vùng ${bx},${by} ${safeW}×${safeH}px)`);
  return true;
}
...
// Click nút cancel trên thumbnail ảnh đang được chọn làm start frame
// HTML: <div class="sc-df80b1f8-4 ..."><i ...>cancel</i></div>
// ═══════════════════════════════════════════════════════════════
...
// Dùng cho vòng 2,3,4 — prompt đã có sẵn trong editor
// ═══════════════════════════════════════════════════════════════
...
(async function main() {
  ...
  log(`Blur vùng    : x=${BLUR_X_PCT}% y=${BLUR_Y_PCT}% w=${BLUR_W_PCT}% h=${BLUR_H_PCT}%`);
  ...
  try {
    ...
    for (let i = 0; i < IMAGE_COUNT; i++) {
      ...
      if (i === 0) {
        // ── Vòng đầu: setup + get prompt + input + submit ──────
        ...
        // ── Vòng 2,3,4: bỏ chọn ảnh cũ → chọn ảnh mới → submit ──
        const ws = randSleep();
    ...
    // Xoá câu lệnh sau khi xong toàn bộ
    log(`→ Xoá câu lệnh...`);
